...
namespace DAO
{
    public class ConjuntoDBText<T> : IConjuntoDB<T> where T : new ()
    {
        ...
        public ConjuntoDBText(string ruta)
        {
            _ruta = ruta;
            _datos = new List<T>();
            CargarDatos();
        ...
        public void CargarDatos()
        {
            ...
            string lineas = ManejadorDeArchivos.LeerArchivo($"{_ruta}\\{archivo}.txt");
            if (!lineas.StartsWith("Exception: "))
            {
                string[] arregloDelineas = lineas.Replace("\r", "").Split("\n".ToCharArray());
                _columnas = arregloDelineas[0];
                string[] nombresColumnas = _columnas.Split("\t".ToCharArray());
        ...
        //REPETIDO EN mANEJADORDESQLSERVER
        ...
        public bool GuardarCambios()
        {
            ...
            return ManejadorDeArchivos.EscribirArchivo($"{_ruta}\\{archivo}.txt", datos);
        }
